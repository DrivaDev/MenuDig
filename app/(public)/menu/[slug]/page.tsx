...
export default async function MenuPage({
  ...
  // Negative top margin for logo overlap on hero
  const logoOverlapClass: Record<string, string> = { sm: '-mt-7', md: '-mt-10', lg: '-mt-14' }
  ...
  return (
    ...
    <div className="menu-theme min-h-screen bg-brand-fondo">
      ...
      {serializedRestaurant.heroImageUrl && (
        <div className="relative w-full h-52 sm:h-72 overflow-hidden">
          ...
          />
          {/* Side gradients */}
          <div className="absolute inset-y-0 left-0 w-12" style={{ background: `linear-gradient(to right, ${menuBgColor}, transparent)` }} />
          <div className="absolute inset-y-0 right-0 w-12" style={{ background: `linear-gradient(to left, ${menuBgColor}, transparent)` }} />
          {/* Bottom gradient — taller to cover logo overlap */}
          <div className="absolute inset-x-0 bottom-0 h-28" style={{ background: `linear-gradient(to top, ${menuBgColor}, transparent)` }} />
        </div>
      )}
      ...
      <div className="sm:max-w-lg md:max-w-2xl sm:mx-auto">
        ...
        {/* Restaurant header */}
        <header className={`bg-brand-fondo px-4 pb-6 sm:px-8 sm:pb-8 flex flex-col gap-3 ${headerAlign} ${serializedRestaurant.heroImageUrl ? '' : 'pt-8 sm:pt-10'}`}>

          {/* Logo — overlaps hero when hero present */}
          {serializedRestaurant.logoUrl && (
            <Image
              ...
              className={`${logoSizeClass[logoSize]} rounded-full object-cover ring-4 ring-brand-fondo border border-brand-acento shrink-0 ${serializedRestaurant.heroImageUrl ? logoOverlapClass[logoSize] : ''}`}
            />
          )}
          ...
          {/* Social links — icons use menuColor */}
          {(serializedRestaurant.whatsappUrl || serializedRestaurant.instagramUrl || serializedRestaurant.facebookUrl || serializedRestaurant.googleMapsUrl) && (
            <div className={`flex gap-4 mt-1 ${logoPosition === 'center' ? 'justify-center' : ''}`}>
              {serializedRestaurant.whatsappUrl && (
                <a href={serializedRestaurant.whatsappUrl} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp">
                  <svg viewBox="0 0 24 24" className="w-7 h-7" style={{ fill: menuColor }} xmlns="http://www.w3.org/2000/svg">
          ...
          {serializedRestaurant.wifiName && serializedRestaurant.wifiPassword && (
            <div className={`flex items-center gap-3 mt-1 py-2 px-3 rounded-lg bg-brand-acento/20 w-fit ${logoPosition === 'center' ? 'self-center' : ''}`}>
              <svg viewBox="0 0 24 24" className="w-5 h-5 shrink-0" style={{ color: menuColor }} fill="none" stroke="currentColor" strokeWidth={2} xmlns="http://www.w3.org/2000/svg">
